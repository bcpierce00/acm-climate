...
should be imposed by governments.  Indeed, many countries and states (e.g.,
California) have already adopted carbon pricing policies; more will follow.
%
Moreover, although the current market price of carbon is still quite low
(around \$15 per ton), experts agree that this will need to rise
significantly over time to achieve the required effect.  Bottom line: We are
entering an era in which carbon is expensive.
...
A possible short-term response to both questions involves {\em carbon
  offsets}---a tool by which organizations (and individuals) can choose to
voluntarily impose a price on their own emissions.
%
% What they are:
In a nutshell, the idea would be that a conference could raise funds from
registration fees or corporate sponsors and give them, via a carbon offset
{\em vendor}, to someone who will use them to fund an activity (e.g.,
installing methane capture devices on landfill sites or buying efficient
cookstoves to replace open-fire cooking in developing areas) that will avoid
...
governments to do it, there are also many things we can do to begin reducing
our footprint.  
\bcp{
  \begin{itemize}
  \item Briefly list a few things we can do (and whose likely effects we'd need data
  to estimate)
  \begin{itemize}
...
However, we urgently need to begin gathering and analyzing data, to
better understand the positive and negative effects of any of these
proposals.   
...
\bcp{Things still missing:
  \begin{itemize}
  \item citations for the claims we make (and the carbon-offsets-bad paper
