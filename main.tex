...
A possible short-term response to both questions involves {\em carbon
  offsets}---a tool by which organizations (and individuals) can voluntarily
impose a price on their own emissions.
%
% What they are:
...
avoids emitting an amount of greenhouse gases equivalent to the emissions
from participants' travel to the conference.
%
Many organizations (including numerous universities, companies such as
Google, Dell, Microsoft, General Motors, Delta Airlines, Expedia, and
others) are already buying carbon offsets; some ACM conferences have already
started to experiment with voluntary carbon offset fees at registration
time.

We acknowledge that there are some significant concerns with carbon offsets.
\begin{itemize}
...
Beyond imposing a price on our carbon footprint (or waiting for one to be
imposed from outside), a sane response to climate change should clearly also
include taking steps to reduce the size of this footprint.  
\bcp{
  \begin{itemize}
  \item Briefly list a few things we can do
  \begin{itemize}
