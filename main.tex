...
dioxide (\COtwoE) per passenger~\cite{carboncalculator}.\iflater\bcp{Trim
  citation?}\fi{} Compare this against the fact that the average US resident
emits about 16.5 tons per year, while the average European emits about 7
tons~\cite{emissions}.  Moreover, these emissions have no near-term
technological fix because jet fuel is difficult to replace with renewable
energy sources~\cite{elec-air}. So reducing the \COtwoE{} footprint of
conferences is going to require a reduction in the footprint of the
...
another obvious step. \mwh{data about surprising connection to conference's
  overall footprint?}\bcp{Or leave it for next round of revisions when we
  may have done more/better analysis?}  Indeed, this switch is already
...
experimentation.  
%
\bcp{Added remainder of paragraph.  I'm not sure the two sentences work
