...
Our first proposal is simple: Every ACM-sponsored conference should publicly
report its carbon footprint (both aggregate and per-participant). Most
conferences' footprints will be dominated by their participants' air travel,
but the accounting should not be limited to that: it should also include air
travel to in-person PC meetings and estimated hotel and food CO2e footprints.

A public accounting will nudge conference organizers and
...
Publicly accounting for CO2e expenditures will incentivize some CO2e
reductions, but probably not major changes to conference system
itself, since doing so may negatively impact the value that
conferences provide.
%
...
cross-fertilization.
%
Organizers and attendees will be reticent to make changes that could
harm the scientific benefits they have all regularly experienced.
%
Nevertheless, it seems clear that the status quo is unsustainable, so
hard choices will have to be made. How to motivate organizers to make
them?

This dilemma is the one that all of society faces, in a microcosm. To
address it, many policy experts advocate some form of carbon tax (or
\emph{price}), which adds a near-term, monetary cost to greenhouse gas
emissions~\cite{carbonprice}. Doing so makes manifest today the hidden
environmental cost of CO2e emissions on tomorrow, thus incentivizing
CO2e-reducing changes without specifying exactly which, thus allowing
for creative and efficient responses. As an analogy, some
municipalities such as New York city and Philadelphia have imposed a
per-calorie tax on soft drinks, with some studies finding that doing
so indeed reduced consumption~\cite{sodatax}.

In the best case, governments would impose a CO2e tax uniformly, and
all CO2e-intensive activities would have to pay it. But ACM could send
a strong message about the importance of this issue by acting now. In
particular, as several companies have shown, even an artificial ``internal price'' on
  carbon creates a significant price signal that shapes behavior.
%
  Thus, our second (more ambitious) proposal is: ACM should begin requiring
  conferences to pay a surcharge based on their carbon footprint.  It should
  be low at the beginning and increase steadily (and predictably) year
  on year.
%
Conference organizers can choose (on the basis of
  careful analysis of data and weighing of benefits) how best to do
  so.

 What should ACM do with the funds collected? The money raised in
  this way can be used in many ways: buying carbon offsets, supporting
  ``green computing research'', virtualizing conferences
  (livestreaming, etc.), supporting a transition to a low-carbon / few
  conferences business model for ACM.

Once again, ACM should provide support to conference organizers to
prepare for a future of higher-cost conferences. 
