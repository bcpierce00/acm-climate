...
ACM can provide tools to measure emissions. As a start, SIGPLAN (the special
interest group on programming languages) recently worked with ACM's SIG
Governing Board to develop an air-travel-focused CO2e calculator for
conferences. Users can upload conference registration data, and the
calculator will estimate the CO2e cost of air travel for those sufficiently
far away. This calculator could be expanded to account for other conference
emissions.
...
Another simple, short-term expedient for lowering net the emissions
footprint of conferences is purchasing \emph{carbon
...
decided to allow conferences to allow attendees to pay for an offset during
...
with corporate sponsorship of bulk offsets for the whole conference.  (It
...
that---especially given their currently quite low cost---they will encourage
complacency, leading organizations to postpone making the difficult changes
that will certainly be required to fully decarbonize society.  We agree that
offsets are at best a short-term measure, to be considered in parallel with,
...
However, while publicly accounting for CO2e expenditures will incentivize
some CO2e reductions, it will probably not, by itself, induce major changes
to the conference system.  There are good reasons for this, since such
changes will undoubtedly impact the value that conferences provide.
%
...
Indeed, a preliminary analysis that we have done with registration data for
recent SIGPLAN conferences raises as many questions as it answers.
\bcp{Talk about our data analysis, and how it could provide help with
  this. We've started, and see that the situation is not very
  clear. Describe some of our data.  Show the two diagrams.  Draw some
  conclusions / discuss why it's hard to draw conclusions.  (This would be a
  good place to refer to~\cite{ClimateCommitteReport}---we can enumerate
  some of the alternatives that one might have hoped the data would help us
  choose between.)}

\bcp{Tricky transition here.  I think the point we want to make is that
  these decisions are genuinely hard and painful, which is exactly what our
  data shows.}
