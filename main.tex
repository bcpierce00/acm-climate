...
Committee\footnote{https://www.sigplan.org/Resources/Climate/} to research
this question.  After investigating many
options~\cite{ClimateCommitteReport} and discussing intensively with SIGPLAN
members and conference organizers, ACM staff, and other SIG leaders, we have
two concrete proposals. First, all ACM conferences should publicly {\em
  account} for CO2e emitted as a result of putting them on. Second, ACM
should put a \emph{price} on carbon in conference budgets, to create
pressure on organizers to reduce their footprints.

\SECTION{Mandate public accountability.}  

Our first proposal is simple: {\bf Every ACM-sponsored conference should
calculate and publicly report its carbon footprint.} Most conferences'
...
discussion has led the organizers of two of its flagship conferences (POPL
and ICFP) to switch from in-person to on-line program committee meetings,
joining a trend among other SIGs, most of which now use virtual PC meetings
for some or all of their flagship conferences. It has also prompted
conferences to increase their investments in livestreaming and video
...
that future organizers can use to optimize their own footprints.  Emissions
data should be published both in aggregate form (total footprint for the
conference and per-participant footprint), for quick reference, and in a raw
...
SIGPLAN recently worked with ACM's SIG Governing Board to develop an
...
\caption{Top: Carbon footprint per participant for travel to recent SIGPLAN
  conferences\iflater\bcp{Get better file from the other paper, or get
    better version of this one from Crista.}\fi.  (Small dots represent
  \bcp{XXX} tons of CO2e per participant; the largest dot represents
  \bcp{YYY} tons per participant.) Bottom: Breakdown of 
  continent-of-origin for participants in each conference. 
...
Figure~\ref{fig:confs} illustrate some of the difficulties.  The top diagram
shows an estimated per-participant CO2e footprint for each instance of the
four major SIGPLAN conferences over the past ten years (excluding a few for
which we had trouble getting data), with larger dots representing more
emissions.  Eyeballing this diagram, it appears that carbon-conscious
conference planners should hold all of these conferences either in the
northeast US or in western Europe every year.  But the bottom visualization
tells a different story.  Here, the horizontal colored bar at the bottom
represents the continent on which each conference was held, and each
vertical bar gives a breakdown of the participants in that conference,
colored according to the continent they gave as their home address.  Again,
from a superficial glance at the colors, it is clear that, though a minority
come from far away, the majority of participants in each case are from the
region where the conference is being held---that is, there is a large amount
of local participation.\iflater\bcp{That point could be strengthened by
  adding some numbers (just in the text, not a figure) about what percentage
  of conference attendees attend even one additional instance of the same
...
require genuinely hard painful compromises.  The impulse to avoid or
postpone thinking about it is entirely understandable!
...
%
Thus, our second proposal is that {\bf ACM should impose a a surcharge on
...
sponsorship, or other, more creative, means.  \iflater\mwh{Problem: We
  showed just above that it's hard to figure out what to do. Come back to
  this point here and say that the surcharge will spur bold choices?  Maybe
  say that these choices won't necessarily be bad, but could be really
  great?}\fi
...
computing research\iflater\bcp{can we mention some specifics?}\fi, and
