...
%
In a nutshell, the idea is that a conference can raise funds from
registration fees or corporate sponsors and give them, via a carbon offset
...
efficient cookstoves to replace open-fire cooking in developing areas) that
...
%
A superficial but potentially sigificant issue is that most
organizations do not currently have accounting procedures in place to deal
with them, and the piecemeal political and bureaucratic work of designing
and implementing a separate policy for each organization can consume
significant energy.
%
A more fundamental problem is that it is not straightforward to decide how
much it should cost to offset a ton of carbon: current prices vary widely
...
implemented, etc..  Worse, in order for offsets (or any other form of a
...
And finally, 
%
We feel that, on balance, voluntary carbon offsetting can be an
effective tool for raising awareness and changing organizational behavior
in the short term.
%
Moreover, supporting well-chosen carbon mitigation activities has a clear
benefit in reducing global net emissions now.
