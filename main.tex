...
starting now, and giving ourselves a ``price signal''
\item Offsets reduce carbon emissions now.  (Yes, there are bogus ones, but
not all are bogus.)
\end{itemize}
\item Concerns:
\end{itemize}
\item It is hard to choose the price (right now there is an oversupply), and setting it too low leads to complacency
\item Organizations are not set up to do it.  Our experience with getting
ACM to introduce an offsetting policy shows that it requires significant
effort.
\end{itemize}
People counter-argue that this "lets people off the hook" and
encourages inaction, but we believe it is consistent with encouraging action
-- again, if offsets are priced right.



The climate crisis is too large and too urgent to just delegate to world
leaders: organizations at every social scale must play their part by addressing
...
organizations you are a part of.  \bcp{Does this smack of incrementalism?
  Maybe delete it.
  ``For example, ask the organizers of the conferences you attend what they
  are doing about reducing or offsetting emissions from participants'
  travel.''}
