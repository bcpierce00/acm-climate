...
\title{\Huge Conferences and Cost of Carbon}
...
In the long term, this cost must be imposed from outside, by governments.
...
over time to achieve the required effect.  Bottom line: carbon is going to
be expensive. 
...
A short-term response to both questions: ACM can choose to voluntarily impose
a price on its emissions using carbon offsets. 
\begin{itemize}
\item (What they are.)
\item giving ourselves a ``price signal''
is an effective way to begin raising awareness and changing organizational
behavior.
\item Lots of organizations (universities, companies) are doing this; some
ACM conferences are starting to experiment with voluntary carbon offset fees
...
Whether we choose to impose carbon offsets on ourselves or wait for
governments to do it, we need to begin gathering and analyzing data, to
...
\bcp{Add some meat to that.
  \begin{itemize}
  \item List a few things we can do (and whose likely effects we'd need data
  to estimate)
  \begin{itemize}
...
\bcp{
\begin{itemize}
\iten Add a bit of meat about how the current economic model may be
