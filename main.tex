...
concerns.
\begin{itemize}
\item Reasons in favor:
\begin{itemize}
\item It is the right thing to do: We should be changing our behavior
starting now, and giving ourselves a ``price signal'' is a good way to
achieve this.
\item Offsets reduce carbon emissions now.  (Yes, there are bogus ones, but
not all are bogus.)
\end{itemize}
\item Concerns:
\begin{itemize}
...
effort.
\item It is hard to choose the price (right now there is an oversupply), and
setting it too low leads to complacency --- it doesn't really bite, so it
doesn't change people's behavior
\end{itemize}
\item People argue that offsets "let people off the hook" and encourage
