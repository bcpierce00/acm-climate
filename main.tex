...
conferences should publicly {\em account} for {\COtwoE} emitted as a result of
...
result~\cite{menu}.  
%MWH: I don't know why this should be a parenthetical.  Why would the analogy be an aside? It is meant to connect the reader's understanding to what we are proposing, explciitly; it is meant to be read, not skipped over.

As a case in point, SIGPLAN has been considering how to reduce {\COtwoE} usage
over the past couple of years, informed by an accounting of its own {\COtwoE}
...
%
SIGPLAN has also explored how to mitigate a conference's {\COtwoE} emissions by
...
The veracity and permanence of this {\COtwoE} emission avoidance is certified by
...
a central place and in a uniform format, to facilitate analysis.  (Most
...
ACM should provide tools to gather and publicize this data. As a
first step, SIGPLAN recently worked with ACM's SIG Governing Board to develop an
air-travel-focused {\COtwoE} calculator for conferences. Users can upload
conference registration data, and the calculator will estimate the {\COtwoE} cost
...
%
While publicly accounting for {\COtwoE} expenditures will incentivize some {\COtwoE}
...
cross-fertilization.
%
Organizers and attendees will be reticent to make particular changes
...
To illustrate the challenge, consider the goal of choosing a conference
location that minimizes {\COtwoE} emissions from participants' travel. 
Using recent conference registration data,
Figure~\ref{fig:confs} relates a conference's location 
to where its participants are coming from.
%
The top diagram shows an
estimated per-participant {\COtwoE} footprint for each instance of the four major
...
environmental cost of {\COtwoE} emissions, thus incentivizing {\COtwoE}-reducing
...
conferences in a virtual setting.  But before any of this, the first concern
...
Beyond simple belt-tightening, carbon pricing and the funding of initiatives
such as this should stimulate creative rethinking of the conference model
...
%
A potential sticking point, however, is that some of these strategies will also
...
that is coming.  ACM should play its part!
