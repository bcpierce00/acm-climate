...
How should ACM respond to these facts? We are the members of SIGPLAN's
(the special interest group on programming languages) Climate
Committee,\footnote{https://www.sigplan.org/Resources/Climate/} and we
have been researching this 
question for the past couple of years. Our work has led to two concrete
proposals. First, all ACM conferences should publicly {\em account} for CO2e
emitted as a result of putting them on. Second, ACM should put a
\emph{price} on carbon in conference budgets, to create pressure on
organizers to reduce their footprints.
...
calculate and publicly report its carbon footprint (both aggregate and
per-participant). Most conferences' footprints will be dominated by
participants air travel, but the accounting should go beyond this to include
air travel to in-person PC meetings and estimated hotel and food CO2e
footprints.
...
SIGPLAN has been considering how to reduce CO2e usage over the past couple
of years, informed by an accounting of its CO2e expenditures. This
discussion has led the organizers of some of its flagship
conferences\mwh{name which conferences}\bcp{POPL and ICFP, not PLDI; I don't
  remember where SPLASH stands} to switch from in-person to on-line program
committee meetings, joining a trend among other SIGs. \mwh{Mention Jens
  survey here.} It has also prompted conferences to increase their
investments in livestreaming and talk recording to permit remote
participation.

SIGPLAN has also been exploring how to mitigate a conference's CO2e emissions
by purchasing \emph{carbon
  offsets}~\cite{CarbonOFfsetReport}.
%
...
decided that conferences could offer attendees the option to pay for an offset during
registration; the conference then uses the designated funds to purchase the
offset from a qualified vendor.  Some conferences are also experimenting
with corporate sponsorship of bulk offsets for the whole conference.\footnote{It
should be noted that carbon offsets are a controversial tool.  Besides
issues with verifiability and permanence, some observers worry
that---especially given their currently artificially-low cost---they will encourage
complacency, leading organizations to postpone making the difficult changes
that will be required to sustainably decarbonize society.  We believe that
offsets are a short-term measure, to be considered in parallel with,
not instead of, more difficult cutting.}

Beyond encouraging conferences to take these sorts of simple steps, public
accounting of conference emissions footprints will generate valuable data
that future organizers can use to optimize their own footprints.
%
\bcp{Talk about the little bit of data gathering that we've done.  Make the
  point that, if the public accounting of conference carbon footprints is
  detailed enough (e.g., including an origin city for each participant,
  rather than just aggregate statistics on average per-participant
  footprint), and if it's publicized in a centralized place and in a uniform
  format, then it becomes a resource for all sorts of useful analysis.}

ACM can provide tools for gathering and publicizing this data. As a start,
SIGPLAN recently
worked with ACM's SIG Governing Board to develop an air-travel-focused CO2e
calculator for conferences. Users can upload conference registration data,
and the calculator will estimate the CO2e cost of air travel for those
sufficiently far away. This calculator could be expanded to account for
other conference emissions.


\SECTION{Put a Price on Carbon.}

While publicly accounting for CO2e expenditures will incentivize
some CO2e reductions, it will probably not, by itself, induce major changes
...
\caption{Top: carbon footprint per participant for travel to recent SIGPLAN
  conferences\iflater\bcp{Get better file from the other paper, or get
    better version of this one from Crista.}\fi.  Bottom: Origin of
  participants for each conference.  \bcp{Explain in more detail, once we
    have Crista's new figures.}} 
\label{fig:confs}
\end{figure}

The registration data we've gathered from SIGPLAN conferences illustrates
...
\bcp{Summary: The data shows why these decisions are genuinely hard and
  painful.}

Nevertheless, it seems clear that the status quo is unsustainable, so hard
choices will have to be made. How do we motivate organizers to make them?
...
corporate sponsorship, or some combination. \mwh{Problem: We showed
  just above that it's hard to figure out what to do. Come back to
  this point here and say that the surcharge will spur bold choices?
  Maybe say that these choices won't necessarily be bad, but could be
  really great?}

What should ACM do with the funds collected from this surcharge?  One can
imagine many good uses, including buying carbon
offsets, supporting ``green'' computing
research\bcp{can we mention some specifics?}, and defraying the costs of
virtualizing conferences (livestreaming, etc.).  \bcp{Expand this paragraph
  a bit.}
