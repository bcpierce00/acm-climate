...
\newif\iflater\latertrue    % comments / todos for final version
...
\bcp{Richard Kim sent a bunch of great comments!  In particular...
  \begin{itemize}
  \item (a) You might say something more about how ACM is in a good position
  to set a trend -- something about taking leadership among scientific
  societies or being on the forefront. There's a little bit about this at
  the end, but it might be developed more into a sentence or paragraph. You
  probably want to phrase this as leadership as opposed to say, sticking
  one's neck out, but there could be something about how computer science and
 tech folks are current and forward-looking, and it's consistent that we
 not be behind the leading edge.
 \item (b) You might say more how ACM needs to prepare itself in light of
 the trends that are coming -- carbon pricing is good organizational sense!
 There's some of this around ACM's potentially needing to adjust its
 business model, but it falls short of making a *business case for carbon
 pricing*, which is what I guess I'm recommending.
 \item Also, in addition to mentioning that Microsoft and other tech
 companies set a price on carbon, you might also note here that Exxon and
 other energy companies set an internal price on carbon as well, as a means
 of risk assessment. In other words, even the oil companies know what's
 coming.
 \end{itemize}
}
...
passenger\iflater\bcp{Citation?}\fi, which is a significant fraction of the
{\em total} yearly emissions for an average resident of the US (16.5 tons)
or Europe (7 tons)~\cite{emissions}.  \iflater\bcp{For comparison, ...  (put
 in some figures about the footprints of a few other things that people
 might think of reducing---driving, eating meat, upgrading their
 fridge...)}\fi{} Moreover, these emissions have no near-term technological
...
the conference. Second, ACM \iflater\bcp{Are we really
 talking about ACM, in the first instance?  Or is the proposal really that
 individual SIGs should do this, until there is enough buy-in to do it at
 the ACM level?  The first is easier to say, but the second is the only
 plausible implementation path.  If there's space for at least a sentence
 or two on this (and other practicalities of implementation), it might be
 good.}\fi should put a \emph{price} on 
...
publicly report its carbon footprint}.  \mwh{CUT: Critically, this accounting should include the emissions generated by participants' travel to and from each conference, as this will constitute the bulk of the conference's carbon footprint.}
%
...
result~\cite{menu}.  
%MWH: I don't know why this should be a parenthetical.  Why would the
%analogy be an aside? It is meant to connect the reader's understanding to
%what we are proposing, explciitly; it is meant to be read, not skipped
%over. 
% BCP: In general, parenthetical remarks are not meant to be skipped over:
% they are meant to be read as asides.  I think this reads better in parens,
...
\PARAGRAPH{Carbon offsets}
%
SIGPLAN has also explored how to mitigate conference emissions by purchasing
\emph{carbon offsets}~\cite{CarbonOFfsetReport}.
%
...
by a watchdog organization. (Typically, planting trees is not a certifiable
...
carbon offsets in order to reduce their 
net footprint. ACM conferences can do the same, and the purchases can be
mentioned in the public accounting we are proposing.  In fact, ACM recently
decided that conferences could offer attendees the option to pay for an
offset during registration.  Other scientific societies are going further,
rolling carbon offsets into the registration fees for all
participants.\iflater\bcp{In a later draft, we may be able to say that ICML,
NIPS, etc. are a case in point.  Ask Alexandre Lacoste about this.}\fi

We acknowledge that carbon offsets are a 
  controversial 
  tool~\cite{anderson2012inconvenient,
  carbon-offsets-are-not-our-get-out-jail-free-card}.  \mwh{CUT: Besides issues with
  verifiability and permanence, some observers worry that---especially at
  their current artificially low cost---they will encourage complacency,
  leading organizations to postpone facing the difficult changes that will
  be required to sustainably decarbonize society.} Offsets are a
useful short-term expedient, not a replacement for more difficult
cutting. 
% MWH: It's a bit odd to have two lengthy parenthetical asides in the same
% paragraph. I think this one is on point, so need not be an aside.
% BCP: The whole paragraph was in parens, before.  However, the new paragraph
% header does a decent job of "closing the aside," so I agree the parens can
% go. 

% \mwh{Should
% we mention the FCRC numbers here? 45 out of 2,700 used {\COtwoE} offsets}
% That's getting too much down into the details, I think -- also, these
% numbers reflect a hastily organized, preliminary version of what we really
% want to do.

\PARAGRAPH{Data gathering}
%
Beyond encouraging conferences to trim their emissions, public 
accounting will generate useful data for future organizers.  \mwh{CUT: To this end,
emissions data should be published both in aggregate 
form (total and per-participant footprints), for quick reference, and in raw
form, with an anonymized record for each participant in each
conference, including their originating city.}  The data should be made
available in 
a central place, in a uniform format.  Most
conferences' footprints will be dominated by participants' air travel, but the
data gathered should go beyond this to include air travel to in-person PC
meetings and estimated hotel and food {\COtwoE} footprints.

ACM should develop tools to gather and publicize this data. As a first step,
SIGPLAN recently built an air-travel-focused {\COtwoE} calculator for
conferences\iflater~\cite{carboncalculator}\bcp{Trim citation?  Is it even
 the right one? Link seems
 dead!  But Richard says ``As a reader, my first question is, how do I get
 access to this calculator? Can you cite this calculator in some way? It
 would be great to give a citation to where people can learn about it or
 use it, even if just, "To learn more about the calculator, e-mail xxx"''}\fi. 
Users can upload conference registration data, and the
calculator will estimate the {\COtwoE} cost of air travel for those
sufficiently far away. This calculator could be expanded to account for
other forms of conference emissions.

We acknowledge that there is some the reputational risk to ACM in taking the
step of publicizing its carbon footprint: the numbers are likely to be high,
and may be used to criticize both the ACM and the broader academic
community.  Nevertheless, making this information available is a crucial
first step: we cannot manage what we have not measured.

\SECTION{Put a Price on Carbon}

While public accounting of {\COtwoE} expenditures will incentivize some 
...
cross-fertilization.
%
Organizers and attendees will be reticent to make changes that might
...
\caption{Top: Carbon footprint per participant for travel to recent SIGPLAN
  ...
  home city is in each continent.  \iflater\bcp{Richard: I find this a
...
\bcp{Richard: I get a little lost in this paragraph. Partly because I found the figure a bit confusing, partly because of the length of the paragraph. I think one fix might be to frontload your conclusion a bit with a topic sentence. E.g., start the paragraph with something like:
"For example, choosing a conference location with the goal of minimizing CO2e emissions from participants' travel results in trade-offs in who participates."}
...
registration data from four SIGPLAN conferences, Figure~\ref{fig:confs}
...
Eyeballing this diagram, it seems that carbon-conscious 
...
continent of their work address.  From a superficial glance
...
conference is being held.\iflater\bcp{That point could be strengthened by adding some
  ...
  conference...}\fi{} This suggests that locating
conferences every year in the same one or two places would significantly
damage the 
research community by discouraging participation from other parts of the
world.\bcp{Richard: The damage to the research community is a little unclear to me on first read. I would reword to something like "that always locating conferences in the same one or two places would significantly damage the heterogeneity of the research community by discouraging participation from other parts of the world". Or perhaps some better world, but more specificity of the damage.}  Indeed, one might conclude that, from the point of view of
...
%MWH: The above is not a parenthetical, so the earlier junkfood analogy should not be, either
...
price'' on carbon sends a significant price signal that shapes behavior.
%
Thus, our second proposal is that {\bf ACM should impose a surcharge on
  conferences based on their carbon footprint.}  The charge should be low to
start and increase steadily and predictably, year on year.
%
