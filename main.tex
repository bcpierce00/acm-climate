...
Exactly what the
cost should be is a challenging question, but it has a clear ``success
...
% (Experts seem to agree that the current price of around \$15 / ton is too
% low.)

Many countries and states (e.g., California) have already adopted carbon
pricing policies; more will follow.  The upshot is that, sooner or later,
carbon is going to be expensive.
\bcp{Yes, this may not happen.  But if it does not, we are probably
  screwed.  So might as well behave as if we knew it was going to happen.}
%
In the meantime, knowing that this is coming, what should we rationally (and
ethically) do now?

First, we should immediately start planning for how ACM, and computer-science
research more broadly, are going to continue to thrive in a world where air
travel is significantly more expensive.  Indeed, this may well involve
rethinking the whole basis of how computer science is done. 
\bcp{Add some meat to that.}

Second, we can begin gathering and analyzing data, to better understand both
the current carbon impact of our conferences and the likely effects (both
environmental and scientific) of potential changes in the way they are
organized.
\bcp{Put in the most interesting discussion and perhaps show an interesting
  diagram.} 

Finally, we can start imposing a cost on ourselves by using carbon
offsets.  
\begin{itemize}
...
\bcp{Things still missing:
  ...
  \item Change to a catchier title (``so something more personal such as
  "Your conference trip and climate change" might get more readers'') 
