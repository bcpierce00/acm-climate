...
This is not a problem for future
generations, but a present-day emergency: The UN's Intergovernmental
...
play since, for many of their members, travel to conferences represents
...
the equivalent of about 1.8 tons of carbon dioxide (\COtwoE) per
passenger~\cite{carboncalculator}, a significant fraction of the
...
over the past few years, informed by an accounting of its own
...
program committee meetings, joining a trend among other SIGs, and
has prompted several conferences to increase investments in
...
by a watchdog organization. (E.g., planting trees is typically not a certifiable
activity, since it's hard to guarantee they won't be cut down, but
...
net footprint. ACM conferences should do the same, and the purchases should be
included in the public accounting we are proposing.  Indeed, ACM recently
...
We acknowledge that carbon offsets are a
  ...
  carbon-offsets-are-not-our-get-out-jail-free-card}.  They should be
regarded as a
useful short-term expedient, not a replacement for more difficult
cutting.  \bcp{Needs a bit of smoothing.}
...
conferences\iflater~\cite{ClimateCommittee}.
...
% \iflater\bcp{That point could be strengthened by adding some
...
same one or two places would significantly damage the heterogeneity of the
...
science, would be to halve actual emissions every decade from now on,
following a recently proposed
``Carbon Law''~\cite{Rockstrom1269}.  \bcp{Read the
  paper!}
...
new ways of doing business in the lower-carbon future that is now upon us.
ACM should do its part by mandating public accounting of conference carbon
footprints and by putting a concrete price on the carbon that they use.
